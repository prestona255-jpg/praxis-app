...
| 11 | `#account` → **redirect `#profile`** (R9a v3.198) · **S-B SWEEP ✓ renderAccountPage + renderOwnProfile + _opPublishControl + 14 `_account*` helpers DELETED (v3.270–272, ~2,133 L; both Sonnet gates: code-CLEAR / no-block). The orphaned `.account-*` CSS (~92 selectors) is SKIPPED (census-ambiguous — live `.account-readermodel`/`.account-card` interleaved) → a dedicated CSS-dead-sweep; `openAccountDeleteConfirm` (delete-account gap) + `renderShelfBookRow` (F7 keep) skipped for Preston's ruling. See `sb-sweep-debt-table.md`.** | — merged into #14 | — | — | — | — |
...
| 19 | `#scan` renderScan (**SCAN round — CLOSED 2026-08-08, v3.269; Preston felt round 4 = full PASS**) | ✅ camera-dark HOUR chrome over live video → light draft-case review (Universal v1.2 tokens; scoped `.scan-surface`; scan camera-chrome + cover-cloth token families in theme.css) · **R-FIRSTSHELF-DUPES Stage 2 (v3.284): a NEW informational state on the tray + review face — "already shelved" (EXACT, filled gold tick + flag) and "may be a copy" (PROBABLE, hollow gold tick + flag). Built on the tray's existing tick geometry and `.spine-flag` typography, gold-role rather than the `.is-lean` gray (gray = unavailable; these stay visible and, for PROBABLE, still add). Before this, `data-owned` was written and read by NOTHING — zero CSS selectors, zero readers — so R3's mark had no visual presence at all. The "Shelve N" count now equals what Shelve will CREATE (EXACT folds are excluded; PROBABLE are not, because they are not blocked either). VISUAL GATE: structurally proven, felt pass PENDING** · **T13/T14 STALE-DRAFT (v3.285, LOCAL+UNPUSHED): TWO net-new visible states on this surface.** (1) `#scan-draftbar` — a gold-bordered glass card on the CAPTURE VIEW (inside `#scan-screen-view`, under `.scan-vf-top`, z-21 beneath the z-50 overlays), naming a pending draft by count + age with RESUME and a two-step DISCARD; it REPLACES `#scan-primer-resume`, deleted in the same commit, so one draft has one choice in one place. Per Preston's FORK RULING B a shelf capture is now REFUSED while that bar is on screen — and the shutter stays visibly LIVE (`disabled false · opacity 1 · filter none · cursor pointer`), the tap brightening the bar instead, because a dead-looking control is the `.is-lean`-gray mistake this round is already undoing. The refusal is gated on the bar being ACTUALLY RENDERED (geometry + resolved style, every clause failing OPEN), never on a flag, so a render bug cannot lock the shutter. (2) `.scan-dc.is-unlooked` — a catalogue-outage state split out of the leaning gray exception: upright, ungrayed, gold-deep "couldn't look up" flag in the `.is-shelved`/`.is-maybe` informational register, plus a run-level note on the review face. A 429 was previously laundered into "needs a look", so a correctly-read spine was blamed for a quota failure. VISUAL GATE: structurally proven at 375×812 with screenshots, **felt pass PENDING**; §9 red-team UNRUN (agent-barred), HALT-tier and unresolved | ✅ **native** — mobile-first, 390 is the acceptance surface (env() insets throughout; the most safe-area-exposed surface); desktop = honest secondary (ISBN/search + drop-zone, no fake viewfinder) | ✅ SC8 EMPTY overlay (coaching) + designed KNOWN-OFFLINE card + the four failure states (CALL-FAILED/EMPTY/TRUNCATED/REFUSED) | ✅ hard — `renderScan` → `buildSignedOutPrompt` (scan is billable authed) | ✅ four SC8 failure states from the shelf-vision stop_reason guard + CALL-FAILED free-retry + soft daily cost cap (30/day, honest refusal, never silent-degrade) |
...
| Import/Capture | **RETIRED (CD-6 Stage 3, v3.257)** — the overlay was orphaned (0 live `.open()` callers since Stage 1); UI + own dictation UI + `commitEntries` + `buildBookSearch` deleted (66,681→22,038 B); segmentation pipeline + transport kept HEADLESS on `window.ImportCapture`, now driven by THE DOOR's paste-split (DOOR-SEG). 0 `.ic-` CSS remain | ✅ retired |
| **THE DOOR** (capture sheet — R-CAPTURE, v3.252) | `initCaptureDoor`/`buildCaptureDoor` (views.js door module); summon = create-corner (bottom-left, stacks above the Shelf FAB on mobile) + Capture nav + ⌘N + `openCaptureDoor()` | ✅ LIGHT sheet over `--scrim` (ruled); 390 bottom-sheet / ≥760 corner-card; note/voice/paste + never-silent context chip + commit-and-stay + CA-1 desk carrying-question + Android text-share. Signed-out: commit gated (field preserved). **CD-6 UNIFICATION (CLOSED at three doors, 2026-07-25): Stage 1 (v3.254) retired the Notebook writeline (`#notebook` → `buildNotebookCatchAffordance`, photo migrated into photo mode); Stage 2 (v3.255) retired the Book-Detail Add-marginalia CREATE composer (the "✎ Add marginalia" button opens THIS door pre-targeted to the book; book-page marg list refreshes on commit, guarded). **Stage 3 (v3.257) retired ImportCapture + PAID DOOR-SEG: paste files raw-as-one, then offers "Split into N?" (local heuristic-gated) → `segmentDoc` on the FILED note → a minimal per-note review (register flip + book chip via `candidateBooks`) → `captureNote` batch-loop under CA-2 (children verified→parent deleted; undo re-files parent first). Red-team CLEAN; 390+desktop+all-mode gated.** **CD-6 CLOSED at three doors (2026-07-25, Option-1 re-scope):** onboarding buildActMargin ruled NOT a CD-6 door (beat 6 of the first-run journey, one caller, already writes through the sole-writer `captureNote`) → UI unification is an ONBOARDING-round item under OB L-1 (held future-state), see `onboarding.md`. Micro-lane CD6-NBK-REFRESH-GUARD (v3.256) SHIPPED: a door commit on `#notebook`/`#book` now skips the in-place refresh while an inline picker/panel is open (`capNotebookHasOpenInline`/`capBookPageHasOpenInline`) so it never tears down in-progress UI. **CD-6 door-set complete at three** (writeline v3.254 · book-marg v3.255 · ImportCapture v3.257). **R-CAPTURE ROUND CLOSED (2026-07-25)** — full felt PASS on the real library; v3.257 desktop split FAILED → closed by v3.258; F2 (v3.259) desktop-only, Edit-open + 390 un-walked; `MOBILE-ALLMODES-WALK` owed. UNVERIFIED grades: `<400ms` · ⌘Enter exercise · offline path (`CAPTURE-OFFLINE-UNEXERCISED`). Acceptance card · felt ledger (actual shape) · debt ledger in `docs/studio/capture.md`.** |
