...
| **THE DOOR** (capture sheet — R-CAPTURE, v3.252) | `initCaptureDoor`/`buildCaptureDoor` (views.js door module); summon = create-corner (bottom-left, stacks above the Shelf FAB on mobile) + Capture nav + ⌘N + `openCaptureDoor()` | ✅ LIGHT sheet over `--scrim` (ruled); 390 bottom-sheet / ≥760 corner-card; note/voice/paste + never-silent context chip + commit-and-stay + CA-1 desk carrying-question + Android text-share. Signed-out: commit gated (field preserved). **CD-6 UNIFICATION IN PROGRESS: Stage 1 (v3.254) retired the Notebook writeline — `#notebook` now opens THIS door (`buildNotebookCatchAffordance`); the writeline's photo capture migrated into the door's photo mode. FOUR doors live: this + 3 legacy (Book-Detail Add-marginalia · ImportCapture · onboarding buildActMargin). Round OPEN** |
