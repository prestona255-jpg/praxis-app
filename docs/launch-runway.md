...
| 5 | **commons `#reader` fencing** | open; draft-sub-body debt (`integrations.js:2456`). |
...
| **B-M-SA — subtheory evidence bottom-sheet safe-area (+momentum)** | a small B-M follow-up slice | R-a's *live* branch (ruled 2026-07-20). The mobile evidence rail `.subtheory-rail.subtheory-rail-mobile-open` is a `position:fixed; bottom:0` sheet with `padding:24px 16px` and no `env(safe-area-inset-bottom)` — the same P4 gap the Bloom-orb and Shelf-select-bar slices fixed, missed because B-M's momentum backfill chased the dead `.st-gutter` mock. Fix: `padding-bottom: calc(24px + env(safe-area-inset-bottom))`; verify `-webkit-overflow-scrolling:touch` in the same slice (it's `overflow-y:auto`). Its own gate + felt delta ("evidence-sheet last row clears the home indicator"). The dead `.st-gutter.subtheory-rail-mobile-open` mock the recon originally named is moot (0 js hits) → folds into S-B's dead-code sweep. |
| **MANIFEST-WARM — warm-ify the PWA splash theme** | its own small slice (manifest.json) | R-b, ruled 2026-07-20. `theme_color`/`background_color` = `#191F33` (pre-amber navy), so the PWA splash is navy while the maskable icon ground is warm. Warm-ify the two manifest tokens to match the amber app. Install/splash-visible; manifest.json only. |
| **MASK-SHELL — precache the maskable icon** | rides the next sw.js touch | R-c, ruled 2026-07-20. `/assets/icon-maskable.svg` isn't in `sw.js` APP_SHELL (OS fetches it at install — works). Add it to APP_SHELL at the next sw.js edit; free on the next cache bump, covers re-install/offline. |
