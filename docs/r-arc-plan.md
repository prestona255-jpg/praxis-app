...
| **T8** | **CACHE BUMP** | Every JS change after a bump needs its own bump or the SW serves stale. | Read `CACHE_VERSION` at commit time, **+1 exactly**. Never target a hardcoded number. ⚠ **CORRECTED at Slice 1:** this row originally said "once at the final push" — which **contradicts the armed hook**. `hooks/pre-commit` rule #3 hard-BLOCKS any staged `.js`/`.css`/`.html` outside `docs/`/`tools/`/`design/` that does not ride an `sw.js` bump. **Every code slice bumps.** Multiple bumps across slices are harmless (the final version is what ships). `--no-verify` is NOT the escape: CLAUDE.md forbids skipping hooks unless Preston asks, and the overnight `--no-verify` carve-out is scoped to overnight batches. |
...
| **T11** | **ARRANGEMENT IS THINKING, NOT DATA** (ratified Preston 2026-07-17, Room charter) | `evidenceLayout` (the Room field's positions) entering Yumi context — never-asked-never-forbidden extends to space: the app never interprets arrangement, and Yumi never sees it. | Grep per Room slice: zero `evidenceLayout` in `assembleContextData` / anywhere in yumi-brain.js; zero clustering/inference code over positions. |
