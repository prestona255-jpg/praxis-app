...
- [ ] SLICE-8 CROSS-DEVICE FIRESTORE LEG (follow-up, not launch-critical; R-ARC Slice 8/9, 2026-07-18) — the "Set aside" tombstone (`dismissReaderThread`, `status='dismissed'`, forward-only) is verified: in-session flip-not-delete + hidden-from-visible-filter, and the tombstone SURVIVES the reload boundary in the durable per-uid store (`praxis_state_<uid>`) — proven live on v3.229 with the d0tester stub. The one UNVERIFIED leg is the TRUE cross-device round-trip (device A dismisses → syncs to Firestore → suppresses on device B), which needs a real SIGNED-IN human session (the stub is signed-out per Firebase, so its reload loads anon state, not the per-uid store; the write persists, the app-side load-restore for a stub does not). Click-path: #profile → "Yumi can go one step further" (opted-in) → a named through-line → "Set aside" → confirm the `status:'dismissed'` tombstone syncs to Firestore and stays gone on device B (or after a localStorage clear + fresh sign-in). Co-verify with RM-SPLAT (same readerModel plumbing).
