...
exports.handler = async function(event) {
  ...
  try {
    ...
    var extractionPrompt =
      'You are looking at a photograph of a bookshelf. Identify the books whose '
      + 'spine or cover text is ACTUALLY LEGIBLE in the image. Work systematically '
      + 'across the whole image, left to right, shelf by shelf, so that no readable '
      + 'spine is skipped. Output a book ONLY if you can read its characters in the '
      + 'image. NEVER complete, correct, or guess a title or author from your '
      + 'knowledge of popular or bestselling books. If a spine is blurry, partial, '
      + 'angled, or too small to read, either omit it or mark it low confidence. '
      + 'Returning fewer books than there are in the photo is correct and expected; '
      + 'an invented title or author is a serious error, a missed book is not. '
      + 'Spine text is often printed across multiple stacked lines -- join those '
      + 'lines into ONE complete title (a title set on two or three lines is still '
      + 'a single book). Never output a fragment of a longer title as its own '
      + 'entry, and list each physical book at most once. '
      ...
      + 'The photo may include non-book objects (speakers, frames, plants, decor) '
